...
data = torch.load("/home/justin94lewis/Documents/DeepTracking/New_Tracking/trainingdata.t7")
...
function trainModel(weights)
  ...
  local randint = torch.IntTensor().random(M)
	local target = getSequence(randint)
	input  = dropoutInput(target)
	table.insert(input, h0)
	-- forward pass
	w:copy(weights)
	output = model:forward(input)
	local cost   = criterion:forward(output, target)-- backward pass
...
for index = start_i,end_i do
  ...
  name = '/home/justin94lewis/Documents/DeepTracking/New_Tracking/weights_model0/'.. index*1000 ..".dat"
  --print(name)
...
torch.save('lin_costs0.t7',COSTS)
end
...
for index = start_i,end_i do
  --print('Loading weights ')
  name = '/home/justin94lewis/Documents/DeepTracking/New_Tracking/weights_model1/'.. index*1000 ..".dat"
  --print(name)
  WEIGHT=torch.load(name)
  total_cost = 0
  number = 100
