...
function Recurrent:__init(step, N)
   ...
   for i=1,N do
      ...
      self.prev_output[i] = torch.Tensor()
   end
...
function Recurrent:forward(input)
   ...
   for i = 1,self.N do
     --append previous self.num estimates to h_i
      local imsize = input[i]:size()
      local previous_output = torch.Tensor(self.num,imsize[2],imsize[3])
      for j = math.max(i-self.num,1),i-1 do
        previous_output[i-j] = self.output[j]
      end
      self.prev_output[i] = previous_output
      local full_hidden = torch.cat(previous_output,self.hidden[i-1],1)
      
      local stepInput  = {full_hidden, input[i]}
...
function Recurrent:backward(input, gradOutput)
  local imsize = input[1]:size()
  local previous_output = torch.Tensor(self.num,imsize[2],imsize[3])
  local full = torch.cat(previous_output,input[self.N+1],1)
   zero(self.gradInput[self.N+1], full)
   for i = self.N,1,-1 do
--      --append previous self.num estimates to h_i
--      local imsize = input[i]:size()
--      local previous_output = torch.Tensor(self.num,imsize[2],imsize[3])
...
--        previous_output[i-j] = self.output[j]
--      end
      local full_hidden = torch.cat(self.prev_output[i],self.hidden[i-1],1)
